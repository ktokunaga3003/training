...
<body>
  ...
  <div style="text-align:center;">
    ...
    <%-- <c:if>タグを使用して、Accountオブジェクトのflag==1の場合は「入金」を、
               flag==2の場合は「出金」を表示 --%>
     <c:if test="${sessionScope.account.flag == 1}">
     入金
     </c:if>
     <c:if test="${sessionScope.account.flag == 2}">
     出金
     </c:if>
  ...
  <div style="text-align:center;">
    <%-- リンクを作成する（「ログアウト」は任意問題2） --%>
    <a href="http://localhost:8080/labs/mod7/accountact.jsp">入出金画面に戻る</a>   |
    <a href="http://localhost:8080/labs/mod7/loginAccount.jsp">ログアウト</a>
  </div>
</body>
</html>
